...
@section('info')
  ...
  <meta name="keywords" content="{{$field->art_tags}}" />
...
@section('content')
  <article class="blogs">
    ...
    <div class="index_about">
      ...
      <p class="box_c"><span class="d_time">发布时间：{{date('Y-m-d',$field->art_time)}}</span><span>编辑：{{$field->art_author}}</span><span>查看次数：{{$field->art_view}}</span></p>
      <ul class="infos">
        {!! $field->art_content !!}
      </ul>
      <div class="keybq">
        <p><span>关键字词</span>：{{$field->art_tags}}</p>
      </div>
      <div class="nextinfo">
        <p>上一篇：
          @if($article['pre'])
          <a href="{{url('/article/'.$article['pre']->art_id)}}">{{$article['pre']->art_title}}</a></p>
          @else
          没有上一篇了
          @endif
        <p>下一篇：
